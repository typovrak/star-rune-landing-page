...
export default function CardSellingPoint({ icon, iconHover, iconColor, title, text, className, isSlide }: ICardSellingPoint) {
	return (
		<Card className={`group/hover flex items-center bg-slate-600 transition-all duration-300 ${isSlide ? "hover:bg-slate-700 border-t-2 rounded-none h-full justify-center" : "hover:rotate-cta hover:bg-slate-700 shadow-lg hover:shadow-xl border-2"}${className ? ` ${className}` : ""}`}>
