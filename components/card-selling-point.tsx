...
export default function CardSellingPoint({ icon, iconHover, iconColor, title, text, className }: ICardSellingPoint) {
	return (
		<Card className={`group bg-slate-600 text-white border-2 hover:bg-slate-700 transition-all duration-300 shadow-lg hover:shadow-2xl hover:rotate-cta${className ? ` ${className}` : ""}`}>
