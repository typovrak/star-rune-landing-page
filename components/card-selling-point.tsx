...
export default function CardSellingPoint({ icon, iconHover, iconColor, title, text, className }: ICardSellingPoint) {
	return (
		<Card className={`group flex items-center bg-slate-600 border-2 hover:bg-slate-700 transition-all duration-300 shadow-lg hover:shadow-xl hover:rotate-cta${className ? ` ${className}` : ""}`}>
