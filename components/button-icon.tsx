...
export default function ButtonIcon({ url, icon, title, className, size = "default", titleClassName, ping }: IButtonIcon) {
	return (
		<ExternalLink
			href={url}
			className={`relative group text-slate-950 flex items-center justify-center pl-[3rem] pr-4 whitespace-nowrap rounded-lg hover:rotate-cta will-change-transform duration-300 shadow-lg hover:shadow-xl ${size === "default" ? "h-10 text-sm" : size === "big" ? "h-12 text-base" : "h-14 text-lg"}${className ? ` ${className}` : ""}`}
		>
