...
export default function ButtonIcon({ url, icon, title, className, size = "default", titleClassName, ping }: IButtonIcon) {
	return (
		<ExternalLink
			href={url}
			className={`relative group text-slate-950 flex items-center justify-center whitespace-nowrap rounded-lg hover:rotate-cta will-change-transform duration-300 shadow-lg hover:shadow-xl ${size === "default" ? "h-10 text-sm pl-[3rem] pr-4" : size === "big" ? "h-12 text-base pl-[3.75rem] pr-5" : "h-14 text-lg pl-[5rem] pr-6"}${className ? ` ${className}` : ""}`}
		>
