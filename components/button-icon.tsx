...
export default function ButtonIcon({ url, icon, title, className, size = "default", titleClassName, ping, type, loading }: IButtonIcon) {
	...
	const childrenContent = (
		<>
			<div className={`absolute left-0 top-0 mt-1 ml-1 bg-white group-[.dark]:bg-black group-[.dark]:text-white text-slate-950 p-[0.35rem] bottom-1 group-hover/hover:w-[calc(100%-0.5rem)] transition-all rounded-md duration-300 ${size === "default" ? "h-8 w-8" : size === "big" ? "h-10 w-10" : "h-12 w-12"}`}>
				{loading ?
					<>
