...
export default function CardClient({ src, names, about, text, source, className }: ICardClient) {
	...
	return (
		<Wrapper
			className={`border-2 bg-white rounded-lg p-6 transition-all duration-300 hover:shadow-sm flex flex-col w-card-client-mobile md:w-card-client gap-6 group ${types[source.type].border} ${types[source.type].shadow}${className ? ` ${className}` : ""}`}
		>
